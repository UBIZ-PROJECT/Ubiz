                ...
                <div id="search-content" class="row z-mgr z-mgl pdt-20">
                    <div class="col-12">
                        <div class="row justify-content-start mgb-10">
                            <div class="col-auto">
                                <label style="min-width: 80px" class="text-primary">Mã đơn hàng</label>
                            </div>
                            <div class="col-auto">
                                @include('components.search_operators_1',['name'=>'so-ord-no'])
                            </div>
                            <div class="col-auto">
                                <textarea style="resize: none" class="input-textarea"
                                          is-change="false" placeholder="" id="sv-ord-no">
                                </textarea>
                            </div>
                        </div>
                        <div class="row justify-content-start mgb-10">
                            ...
                            <div class="col-auto">
                                @include('components.search_operators_2',['name'=>'so-ord-date'])
                            </div>
                            <div class="col-auto">
                                <input type="text" style="width: 100px"
                                       value="" name="sv-f-ord-date" autocomplete="off"
                            ...
                            <div class="col-auto">
                                <input type="text" style="width: 100px"
                                       value="" name="sv-t-ord-date" autocomplete="off"
                                       class="date-picker form-control light-color custom-form-control">
                            </div>
                        </div>
                        <div class="row justify-content-start mgb-10">
                            ...
                            <div class="col-auto">
                                @include('components.search_operators_2',['name'=>'so-ord-exp-date'])
                            </div>
                            <div class="col-auto">
                                <input type="text" style="width: 100px"
                                       value="" name="sv-f-ord-exp-date" autocomplete="off"
                                       class="date-picker form-control light-color custom-form-control">
                            </div>
                            <div class="col-auto z-pdl z-pdr">
                                <label>~</label>
                            ...
                            <div class="col-auto">
                                <input type="text" style="width: 100px"
                                       value="" name="sv-t-ord-exp-date" autocomplete="off"
                                       class="date-picker form-control light-color custom-form-control">
                            </div>
                        </div>
                        @if(sizeof($users) > 0)
                        ...
                        <div class="row justify-content-start mgb-10">
                            ...
                            <div class="col-auto">
                                @include('components.search_operators_4',['name'=>'so-ord-amount-tax'])
                            </div>
                            <div class="col-auto">
                                <input type="text" style="width: 240px" value="" name="sv-ord-amount-tax"
                        ...
                        <div class="row justify-content-start mgb-10">
                            ...
                            <div class="col-auto">
                                @include('components.search_operators_5',['name'=>'so-ord-note'])
                            </div>
                            <div class="col-auto">
                                <textarea style="width: 240px" value="" name="sv-ord-note"
...
<script>
    ...
    function clear_advance_searh_form() {
        var tags = $('#sv-ord-no').tagEditor('getTags')[0].tags;
        for (i = 0; i < tags.length; i++) {
            $('#sv-ord-no').tagEditor('removeTag', tags[i]);
        }
        $(".so-drd").val("");
        $("input[name=sv-f-ord-date]").val("");
        $("input[name=sv-t-ord-date]").val("");
        $("input[name=sv-f-ord-exp-date]").val("");
        $("input[name=sv-t-ord-exp-date]").val("");
        ...
        $("input[name=sv-ord-amount-tax]").val("");
        $("textarea[name=sv-ord-note]").val("");
    ...
    function get_search_cond() {
        ...
        //get ord-no search cond
        cond.search_name = 'ord-no';
        cond.search_value = $('#sv-ord-no').tagEditor('getTags')[0].tags;
        cond.search_operator = $('select[name=so-ord-no]').val();
        if (cond.search_operator != '' && cond.search_value.length > 0) {
            search_cond.push(cond);
        }

        //get ord-date search cond
        cond = {};
        cond.search_name = 'ord-date';
        cond.search_operator = $('select[name=so-ord-date]').val();
        cond.search_value = {};
        cond.search_value.from = $("input[name=sv-f-ord-date]").val();
        cond.search_value.to = $("input[name=sv-t-ord-date]").val();
        ...
        //get ord-exp-date search cond
        cond = {};
        cond.search_name = 'ord-exp-date';
        cond.search_operator = $('select[name=so-ord-exp-date]').val();
        cond.search_value = {};
        cond.search_value.from = $("input[name=sv-f-ord-exp-date]").val();
        cond.search_value.to = $("input[name=sv-t-ord-exp-date]").val();
        if (cond.search_operator != ''
            && (cond.search_value.from != '' || cond.search_value.to != '')
        ) {
            search_cond.push(cond);
        ...
        //get ord-amount-tax search cond
        cond = {};
        cond.search_name = 'ord-amount-tax';
        cond.search_value = numeral($('input[name=sv-ord-amount-tax]').val()).format('0');
        cond.search_operator = $('select[name=so-ord-amount-tax]').val();
        if (cond.search_operator != '' && cond.search_value != '') {
            search_cond.push(cond);
        }

        //get ord-note search cond
        cond = {};
        cond.search_name = 'ord-note';
        cond.search_value = $('textarea[name=sv-ord-note]').val();
        cond.search_operator = $('select[name=so-ord-note]').val();
        if (cond.search_operator != '') {
            search_cond.push(cond);
        }
    ...
    jQuery(document).ready(function () {
        ...
        $("#sv-ord-no").tagEditor({forceLowercase: false});
