...
class PricingController extends Controller
{
	...
	public function insertPricing(Request $request)
    {
        try {
			...
			$pricings = $pricing->getPricing(0);
            $paging = $pricing->getPagingInfo();
            $paging['page'] = 0;
			foreach($pricings as $key => $item){
				$pricingProduct = $pricing->getPricingProduct($item->pri_id);
				$pricings[$key]->product = $pricingProduct;
			}
        } catch (\Throwable $e) {
            throw $e;
        }
	...
	public function updatePricing(Request $request)
    {
        try {
            ...
            $paging['page'] = 0;
			foreach($pricings as $key => $item){
				$pricingProducts = $pricing->getCustomerAddress($item->cus_id);
